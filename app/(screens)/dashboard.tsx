...
import type { Note, NoteDocType } from '../../src/types';
...
export default function Dashboard() {
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      Toast.show({ type: 'error', text1: 'Failed to load repository notes data.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const launchAiStudyMode = async (note: Note) => {
    ...
    setStudyMode(false);
    Toast.show({ type: 'info', text1: 'Analyzing notes & prompting Gemini AI...' });
    ...
    try {
      ...
      if (res.data?.quiz && Array.isArray(res.data.quiz)) {
        ...
        setStudyMode(true);
        startTimer(60);
        Toast.show({ type: 'success', text1: 'AI Assessment generated successfully!' });
      } else {
        throw new Error('Malformed payload signature returned from generator.');
      }
    } catch (error: any) {
      const systemErrorMessage =
        error.response?.data?.message || 'AI Engine busy. Please try spinning it up again.';
  ...
  const semesterCounts = (source: UploaderSource) => {
    ...
    SEMESTERS.forEach((sem) => {
      counts[sem] = notes.filter(
        (n) => n.semester === sem && (source === 'expert' ? isExpertUploader(n) : !isExpertUploader(n))
      ).length;
    });
    return counts;
  };
  ...
  const subjectCounts = (semester: number, source: UploaderSource) => {
    ...
    notes
      .filter((n) => n.semester === semester && (source === 'expert' ? isExpertUploader(n) : !isExpertUploader(n)))
      .forEach((n) => {
  ...
  const depthIndex =
    view.level === 'source' ? 0 : view.level === 'semesters' ? 1 : view.level === 'subjects' ? 2 : 3;
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.banner}>
          ...
          <Text style={styles.bannerSubtitle}>
            Descend through semesters and subjects to surface notes, papers, and AI-generated
            revision quizzes.
          </Text>

          <Link href="/expertList" asChild>
            <TouchableOpacity style={styles.expertsLinkCard}>
              ...
              <View style={styles.flex1}>
                ...
                <Text style={styles.expertsLinkSubtitle}>Open real-world secure chats</Text>
              </View>
            </TouchableOpacity>
          </Link>
        </View>
        ...
        {/* Depth gauge */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.depthGauge}>
          {DEPTH_LABELS.map((label, i) => {
            ...
            if (i === 2 && (view.level === 'subjects' || view.level === 'subject-detail'))
            ...
            return (
              <React.Fragment key={label}>
                <TouchableOpacity
                  ...
                  style={[
                    styles.depthPill,
                    isActive && { backgroundColor: `${accent}22`, borderColor: `${accent}66` },
                  ]}
                >
                  <View
                    style={[
                      styles.depthDot,
                      { backgroundColor: isActive ? accent : isPassed ? '#71717a' : '#3f3f46' },
                    ]}
                  />
                  <Text
                    style={[
        ...
        {view.level === 'source' && (
          <View style={styles.section}>
            ...
            <TouchableOpacity
              ...
              style={[styles.sourceCard, { borderColor: 'rgba(245,158,11,0.15)' }]}
            >
              <View style={[styles.sourceIconBox, { backgroundColor: 'rgba(245,158,11,0.1)' }]}>
                <User size={20} color={AMBER} />
              </View>
              <Text style={styles.sourceTitle}>Student Uploads</Text>
              <Text style={styles.sourceSubtitle}>Notes and papers shared by fellow students.</Text>
              <View style={styles.sourceFooterRow}>
                <View style={[styles.docCountPill, { backgroundColor: 'rgba(245,158,11,0.1)', borderColor: 'rgba(245,158,11,0.2)' }]}>
                  <Text style={{ color: AMBER, fontSize: 10, fontWeight: '700' }}>
                    {loading ? '...' : `${notes.filter((n) => !isExpertUploader(n)).length} DOCS`}
                  </Text>
                </View>
            ...
            <TouchableOpacity
              ...
              style={[styles.sourceCard, { borderColor: 'rgba(34,211,238,0.15)' }]}
            >
              <View style={[styles.sourceIconBox, { backgroundColor: 'rgba(34,211,238,0.1)' }]}>
                <Award size={20} color={CYAN} />
              </View>
              <Text style={styles.sourceTitle}>Expert Uploads</Text>
              <Text style={styles.sourceSubtitle}>Authoritative notes and papers from experts and admins.</Text>
              <View style={styles.sourceFooterRow}>
                <View style={[styles.docCountPill, { backgroundColor: 'rgba(34,211,238,0.1)', borderColor: 'rgba(34,211,238,0.2)' }]}>
                  <Text style={{ color: CYAN, fontSize: 10, fontWeight: '700' }}>
                    {loading ? '...' : `${notes.filter((n) => isExpertUploader(n)).length} DOCS`}
                  </Text>
                </View>
        ...
        {view.level === 'semesters' && (
          <View style={styles.section}>
            <View style={styles.sectionHeaderRow}>
              <Layers3 color={view.source === 'expert' ? CYAN : AMBER} size={18} />
            ...
            {SEMESTERS.map((sem) => {
              ...
              return (
                <TouchableOpacity
                  key={sem}
                  onPress={() => setView({ level: 'subjects', source: view.source, semester: sem })}
                  style={[styles.semesterCard, { borderColor: `${accent}22` }]}
                >
                  <View style={[styles.sourceIconBox, { backgroundColor: `${accent}1A` }]}>
                    <Layers3 size={20} color={accent} />
                  </View>
                  <Text style={styles.sourceTitle}>Semester {sem}</Text>
                  <Text style={styles.sourceSubtitle}>{SEMESTER_SUBJECTS[sem].length} subjects</Text>
                  <View style={styles.sourceFooterRow}>
                    <View style={[styles.docCountPill, { backgroundColor: `${accent}1A`, borderColor: `${accent}33` }]}>
                      <Text style={{ color: accent, fontSize: 10, fontWeight: '700' }}>
                        {loading ? '...' : `${semesterCounts(view.source)[sem] || 0} DOCS`}
                      </Text>
                    </View>
        ...
        {view.level === 'subjects' && (
          <View style={styles.section}>
            <View style={styles.sectionHeaderRow}>
              <BookOpen color={view.source === 'expert' ? CYAN : AMBER} size={18} />
              <Text style={styles.sectionTitle}>Semester {view.semester} Subjects</Text>
            </View>
            ...
            {SEMESTER_SUBJECTS[view.semester].map((subj) => {
              ...
              return (
                ...
                >
                  <View style={[styles.subjectCodeBadge, { borderColor: `${accent}44` }]}>
                    <Text style={{ color: accent, fontSize: 11, fontWeight: '800' }}>{subj.code}</Text>
                  </View>
                  ...
                  <View style={styles.rowGap}>
                    {loading ? (
                      ...
                      <>
                        ...
                        {total === 0 && <Text style={styles.emptyItalic}>empty</Text>}
                      </>
                    )}
                  </View>
                </TouchableOpacity>
          ...
          <View style={styles.section}>
            <View
              style={[
                ...
                { alignSelf: 'flex-start', borderColor: view.source === 'expert' ? `${CYAN}44` : `${AMBER}44` },
              ]}
            >
              <Text style={{ color: view.source === 'expert' ? CYAN : AMBER, fontSize: 11, fontWeight: '800' }}>
            ...
            <View style={styles.tabsRow}>
              <TouchableOpacity
                ...
                style={[styles.tabChip, activeTab === 'note' && { backgroundColor: AMBER }]}
              >
                <NotebookText size={13} color={activeTab === 'note' ? '#000' : '#71717a'} />
                <Text style={[styles.tabChipText, activeTab === 'note' && { color: '#000' }]}>Notes</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => setActiveTab('paper')}
                style={[styles.tabChip, activeTab === 'paper' && { backgroundColor: CYAN }]}
              >
                <FileText size={13} color={activeTab === 'paper' ? '#000' : '#71717a'} />
                <Text style={[styles.tabChipText, activeTab === 'paper' && { color: '#000' }]}>Papers</Text>
              </TouchableOpacity>
            </View>
            ...
            {loading ? (
              ...
              <View style={styles.emptyDocsBox}>
                ...
                <Text style={styles.emptyDocsTitle}>No {activeTab === 'note' ? 'notes' : 'papers'} yet</Text>
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.docCard,
                      isSelected && {
                        borderColor: activeTab === 'paper' ? `${CYAN}80` : `${AMBER}80`,
                        backgroundColor: activeTab === 'paper' ? 'rgba(34,211,238,0.04)' : 'rgba(245,158,11,0.04)',
                      },
                    ]}
                  >
                    <View style={styles.docCardTop}>
                      ...
                      <View
                        style={[
                          styles.docTypeBadge,
                          {
                            backgroundColor: activeTab === 'paper' ? 'rgba(34,211,238,0.1)' : 'rgba(245,158,11,0.1)',
                            borderColor: activeTab === 'paper' ? 'rgba(34,211,238,0.2)' : 'rgba(245,158,11,0.2)',
                          },
                        ]}
                      >
                        <Text
                    ...
                    <View style={styles.docMetaRow}>
                      <View style={styles.rowGapSmall}>
                        ...
                        <Text style={styles.docMetaText}>
                          {note.uploadedBy?.name || (isExpertUpload ? 'Specialist' : 'Peer Student')}
                        </Text>
                      </View>
                      <View style={styles.rowGapSmall}>
        ...
        <View style={styles.section}>
          ...
          {requestsLoading ? (
            ...
            <Text style={styles.emptyInline}>No active requests logged in your tracking history.</Text>
          ) : (
            requests.map((req) => {
              ...
              return (
                <View
                  ...
                  style={[
                    styles.requestCard,
                    isFulfilled && { borderColor: 'rgba(52,211,153,0.25)', backgroundColor: 'rgba(52,211,153,0.03)' },
                  ]}
                >
                  <View style={styles.requestTopRow}>
                    <View style={styles.semBadge}>
                      <Text style={styles.semBadgeText}>SEM {req.semester}</Text>
                    </View>
                    {isFulfilled ? (
                      <View style={styles.fulfilledBadge}>
                        <CheckCircle2 size={11} color={EMERALD} />
                        <Text style={{ color: EMERALD, fontSize: 10, fontWeight: '700' }}>Fulfilled</Text>
                      </View>
                    ) : (
                      <View style={styles.pendingBadge}>
                        <Text style={{ color: AMBER, fontSize: 10, fontWeight: '700' }}>Pending Help</Text>
                      </View>
                    )}
                  </View>
                    ...
                    >
                      <View style={styles.rowGapSmall}>
                        ...
                        <View>
                          ...
                          <Text style={styles.fulfilledNoteSubtitle}>Uploaded by Specialist</Text>
                        </View>
                      </View>
                  ...
                  {isFulfilled && req.fulfilledBy && (
                    <TouchableOpacity
                      onPress={() =>
                        setActiveChatUser({ id: req.fulfilledBy!._id, name: req.fulfilledBy!.name })
                      }
                      style={styles.chatWithSpecialistButton}
                    >
                      <MessageSquare size={13} color="#000" />
                      <Text style={styles.chatWithSpecialistText}>Chat with Specialist</Text>
                    </TouchableOpacity>
                  )}
                </View>
              );
          ...
          {activeNote ? (
            <>
              <View
                style={[
                  styles.subjectCodeBadge,
                  {
                    ...
                    borderColor: activeNote.docType === 'paper' ? `${CYAN}44` : `${AMBER}44`,
                  },
                ]}
              >
                <Text
              ...
              {activeNote.files?.map((link, i) => (
                <TouchableOpacity key={i} style={styles.downloadButton} onPress={() => Linking.openURL(link)}>
                  <Download size={14} color="#e4e4e7" />
                  <Text style={styles.downloadButtonText}>Download Document Attachment</Text>
                </TouchableOpacity>
              ))}
              ...
              >
                ...
                <Text style={styles.quizButtonText}>
                  {generatingQuiz ? 'Generating Smart Exam...' : 'Launch Gemini Revision Quiz'}
                </Text>
              </TouchableOpacity>
                ...
                <View style={styles.quizPanel}>
                  <View style={styles.timerRow}>
                    ...
                    <Text
                      style={[
                        styles.timerValue,
                        timeLeft <= 15 && { color: ROSE, backgroundColor: 'rgba(244,63,94,0.15)' },
                      ]}
                    >
                      {formatTime(timeLeft)}
                    </Text>
                      ...
                      {question.a.map((answer, aIdx) => {
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            onPress={() => setSelectedAnswers((prev) => ({ ...prev, [qIdx]: aIdx }))}
                  ...
                  {quizScore === null ? (
                    <TouchableOpacity onPress={() => evaluateQuiz(false)} style={styles.submitButton}>
                      <Text style={styles.submitButtonText}>Submit Responses</Text>
                    </TouchableOpacity>
                  ) : (
                    <View style={styles.scoreBox}>
                      <Text style={styles.scoreLabel}>Your Assessment Target Score</Text>
                      <Text style={styles.scoreValue}>
                        {quizScore} <Text style={styles.scoreValueMax}>/ {quizQuestions.length}</Text>
                      </Text>
                      <TouchableOpacity
                        onPress={() => {
                          setStudyMode(false);
            ...
            <View style={styles.noModuleBox}>
              ...
              <Text style={styles.noModuleSubtitle}>Select a note or paper from a subject to start.</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  banner: { padding: 24, paddingTop: 50, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' },
  bannerBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  ...
  bannerBadgeText: { color: AMBER, fontSize: 10, fontWeight: '700', letterSpacing: 1.5 },
  bannerTitle: { color: '#fafafa', fontSize: 32, fontWeight: '900', marginTop: 12 },
  bannerSubtitle: { color: '#71717a', fontSize: 13, marginTop: 10, lineHeight: 19 },
  ...
  depthPillText: { color: '#3f3f46', fontSize: 10, fontWeight: '700', textTransform: 'uppercase' },
  depthLine: { height: 1, width: 32, backgroundColor: '#27272a', marginHorizontal: 4, alignSelf: 'center' },
  ...
  sourceIconBox: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
  sourceTitle: { color: '#f4f4f5', fontSize: 16, fontWeight: '700' },
  sourceSubtitle: { color: '#71717a', fontSize: 12, marginTop: 6, lineHeight: 17 },
  sourceFooterRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 },
  docCountPill: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999, borderWidth: 1 },
  ...
  subjectCard: { backgroundColor: '#0a0a0c', borderWidth: 1, borderRadius: 18, padding: 16, marginTop: 12 },
  subjectCodeBadge: {
    backgroundColor: '#000',
    borderWidth: 1,
    alignSelf: 'flex-start',
  ...
  subjectName: { color: '#f4f4f5', fontSize: 14, fontWeight: '700', marginBottom: 12 },
  ...
  emptyDocsBox: { alignItems: 'center', paddingVertical: 40, backgroundColor: '#0a0a0c', borderRadius: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
  emptyDocsTitle: { color: '#a1a1aa', fontSize: 13, fontWeight: '700', marginTop: 10 },
  emptyDocsSubtitle: { color: '#3f3f46', fontSize: 11, marginTop: 4, textAlign: 'center', paddingHorizontal: 30 },

  docCard: { backgroundColor: '#0a0a0c', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 18, padding: 14, marginTop: 10 },
  ...
  docTypeBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1 },
  docMetaRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 14, marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)' },
  docMetaText: { color: '#71717a', fontSize: 10.5 },

  requestCard: { backgroundColor: '#0a0a0c', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 18, padding: 16, marginTop: 12 },
  requestTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  semBadge: { backgroundColor: '#000', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  semBadgeText: { color: '#71717a', fontSize: 9, fontWeight: '700' },
  fulfilledBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(52,211,153,0.1)', borderWidth: 1, borderColor: 'rgba(52,211,153,0.2)', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  pendingBadge: { backgroundColor: 'rgba(245,158,11,0.1)', borderWidth: 1, borderColor: 'rgba(245,158,11,0.2)', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  requestTitle: { color: '#f4f4f5', fontSize: 14, fontWeight: '700' },
  requestSubject: { color: CYAN, fontSize: 11, fontWeight: '700', marginTop: 2, marginBottom: 6 },
  requestDescription: { color: '#71717a', fontSize: 12, lineHeight: 17, marginBottom: 12 },
  fulfilledNoteRow: {
    flexDirection: 'row',
  ...
  chatWithSpecialistButton: { flexDirection: 'row', gap: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', paddingVertical: 10, borderRadius: 12 },
  chatWithSpecialistText: { color: '#000', fontSize: 11, fontWeight: '700' },

  notePanel: { backgroundColor: '#0a0a0c', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 22, padding: 20, marginHorizontal: 16, marginTop: 8 },
  activeNoteTitle: { color: '#f4f4f5', fontSize: 17, fontWeight: '900', marginTop: 8 },
  activeNoteSubject: { color: '#71717a', fontSize: 11, fontWeight: '700', marginTop: 2 },
  scopeBox: { backgroundColor: '#000', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 12, marginTop: 14 },
  scopeLabel: { color: '#3f3f46', fontSize: 9.5, fontWeight: '800', marginBottom: 4 },
  scopeText: { color: '#a1a1aa', fontSize: 12, lineHeight: 18 },
  downloadButton: { flexDirection: 'row', gap: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#000', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', paddingVertical: 12, borderRadius: 12, marginTop: 12 },
  downloadButtonText: { color: '#e4e4e7', fontSize: 12, fontWeight: '700' },
  quizButton: { flexDirection: 'row', gap: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: AMBER, paddingVertical: 13, borderRadius: 12, marginTop: 10 },
  quizButtonText: { color: '#000', fontSize: 12, fontWeight: '700' },
  buttonDisabled: { opacity: 0.4 },

  quizPanel: { marginTop: 20, paddingTop: 18, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)' },
  timerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#000', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 14, padding: 12 },
  timerLabel: { color: '#71717a', fontSize: 11, fontWeight: '700' },
  timerValue: { color: AMBER, fontSize: 14, fontWeight: '900', backgroundColor: 'rgba(245,158,11,0.1)', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 },

  questionBlock: { marginTop: 18 },
  questionHeaderRow: { flexDirection: 'row', gap: 6, marginBottom: 8 },
  questionText: { color: '#e4e4e7', fontSize: 12, fontWeight: '800', flex: 1, lineHeight: 17 },
  choiceRow: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderRadius: 12, padding: 10, marginBottom: 6 },
  choiceDefault: { borderColor: 'rgba(255,255,255,0.08)', backgroundColor: 'rgba(0,0,0,0.3)' },
  choiceSelected: { borderColor: 'rgba(245,158,11,0.5)', backgroundColor: 'rgba(245,158,11,0.06)' },
  choiceCorrect: { borderColor: 'rgba(52,211,153,0.6)', backgroundColor: 'rgba(52,211,153,0.1)' },
  choiceWrong: { borderColor: 'rgba(244,63,94,0.6)', backgroundColor: 'rgba(244,63,94,0.1)' },
  choiceText: { color: '#d4d4d8', fontSize: 11.5, flex: 1 },
  radioOuter: { width: 16, height: 16, borderRadius: 8, borderWidth: 1.5, borderColor: '#52525b', alignItems: 'center', justifyContent: 'center' },
  radioInner: { width: 8, height: 8, borderRadius: 4, backgroundColor: AMBER },

  submitButton: { backgroundColor: '#fff', paddingVertical: 12, borderRadius: 12, alignItems: 'center', marginTop: 14 },
  submitButtonText: { color: '#000', fontWeight: '700', fontSize: 12 },
  scoreBox: { backgroundColor: '#000', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderRadius: 16, padding: 16, alignItems: 'center', marginTop: 14 },
  ...
  resetButton: { marginTop: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', backgroundColor: '#0a0a0c', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10 },
  resetButtonText: { color: '#d4d4d8', fontSize: 11, fontWeight: '700' },

  noModuleBox: { alignItems: 'center', paddingVertical: 40 },
  noModuleTitle: { color: '#71717a', fontSize: 13, fontWeight: '700', marginTop: 10 },
  noModuleSubtitle: { color: '#3f3f46', fontSize: 11, marginTop: 4, textAlign: 'center', paddingHorizontal: 30 },
});