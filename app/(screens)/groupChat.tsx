...
import {
  Send, Smile, Camera as CameraIcon, FileUp, Users,
...
export default function GroupChat() {
  ...
  const processMessageDeletion = (messageId: string, isSelfDrop: boolean) => {
    ...
    socketRef.current?.emit('delete_group_message', { messageId, userEmail: user.email });
    Toast.show({
      type: 'success',
      text1: isSelfDrop ? 'Your message was deleted.' : 'Message dropped by Admin override.',
    });
  ...
  const handleSnapshotCapture = async () => {
    ...
    try {
      ...
      socketRef.current?.emit('send_group_message', payload);
      Toast.show({ type: 'success', text1: 'Snapshot transmitted successfully.' });
    } catch (err) {
  ...
  const processExternalFileAttachment = async () => {
    ...
    Toast.show({ type: 'info', text1: `Uploading asset file: ${asset.name}...` });
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.headerBar}>
        <View style={styles.rowGap}>
          ...
          <View>
            <Text style={styles.headerTitle}>Public Sync Communication Channel</Text>
            <Text style={styles.headerSubtitle}>NoteVault low-latency grid connection</Text>
          </View>
        </View>
        <View style={styles.nodeIdChip}>
          <MonitorSmartphone size={12} color={AMBER} />
          <Text style={styles.nodeIdText}>{user?.name || 'anonymous_peer'}</Text>
        </View>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item: m }) => {
          ...
          return (
            <View style={[styles.messageBlock, isSelf ? styles.messageBlockSelf : styles.messageBlockOther]}>
              <View style={styles.senderMetaRow}>
                <View style={styles.roleTag}>
                  <Text style={styles.roleTagText}>{senderRole}</Text>
                </View>
                <Text style={isSelf ? styles.senderNameSelf : styles.senderNameOther}>{m.sender?.name}</Text>
                {isAdminNode && (
                ...
                {editingId === messageId ? (
                  <View style={styles.editRow}>
                    <TextInput
                      ...
                      autoFocus
                    />
                    <TouchableOpacity onPress={() => submitMessageUpdate(messageId)}>
                  ...
                  <>
                    {!!m.text && (
                      <Text style={isSelf ? styles.bubbleTextSelf : styles.bubbleTextOther}>{m.text}</Text>
                    )}
                    {!!m.emoji && <Text style={styles.emojiMessage}>{m.emoji}</Text>}
                    {!!m.cameraSnapshot && (
                      <Image source={{ uri: m.cameraSnapshot }} style={styles.snapshotImage} />
                    )}
                    {!!m.fileUrl && (
                      <TouchableOpacity
              ...
              {isMenuOpen && editingId !== messageId && (
                <View style={[styles.actionRow, isSelf && { alignSelf: 'flex-end' }]}>
                  {!!m.text && (
                    <TouchableOpacity onPress={() => handleTextCopy(m.text || '')} style={styles.actionButton}>
      ...
      {/* Camera Modal */}
      <Modal visible={liveCamera} animationType="slide" onRequestClose={() => setLiveCamera(false)}>
        <View style={styles.cameraScreen}>
          <TouchableOpacity onPress={() => setLiveCamera(false)} style={styles.cameraCloseButton}>
            <X size={20} color="#fff" />
          </TouchableOpacity>
          {permission?.granted && (
            <CameraView ref={cameraRef} style={styles.cameraView} facing="front" />
          )}
          <TouchableOpacity onPress={handleSnapshotCapture} style={styles.captureButton}>
      ...
      {showEmojis && (
        <View style={styles.emojiTray}>
          {REACTION_STAMPS.map((stamp) => (
            <TouchableOpacity key={stamp} onPress={() => handleStampInjection(stamp)} style={styles.emojiStampButton}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.inputBar}>
          <TouchableOpacity onPress={processExternalFileAttachment} style={styles.iconBarButton}>
          ...
          <TouchableOpacity
            ...
            style={[styles.iconBarButton, showEmojis && styles.iconBarButtonActive]}
          >
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { color: '#71717a', fontSize: 9, marginTop: 2, textTransform: 'uppercase' },
  nodeIdChip: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  senderMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4, paddingHorizontal: 2 },
  roleTag: { backgroundColor: '#18181b', borderWidth: 1, borderColor: '#27272a', paddingHorizontal: 5, paddingVertical: 1, borderRadius: 4 },
  roleTagText: { color: '#71717a', fontSize: 8, fontWeight: '700', textTransform: 'uppercase' },
  senderNameSelf: { color: AMBER, fontSize: 10, fontWeight: '700' },
  senderNameOther: { color: '#a1a1aa', fontSize: 10 },
  adminBadge: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: 'rgba(153,27,27,0.3)', borderWidth: 1, borderColor: 'rgba(153,27,27,0.5)', paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4 },
  ...
  bubbleOther: { backgroundColor: '#121216', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', borderTopLeftRadius: 4 },
  bubbleTextSelf: { color: '#000', fontSize: 13, lineHeight: 18, fontWeight: '500' },
  ...
  fileChip: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#000', borderWidth: 1, borderColor: '#27272a', padding: 8, borderRadius: 10, marginTop: 6 },
  fileChipText: { color: '#fff', fontSize: 11, fontWeight: '700', flexShrink: 1 },

  editRow: { flexDirection: 'row', alignItems: 'center', gap: 10, minWidth: 180 },
  editInput: { flex: 1, color: '#000', borderBottomWidth: 1, borderBottomColor: 'rgba(0,0,0,0.4)', fontSize: 13, paddingVertical: 2 },
  ...
  cameraScreen: { flex: 1, backgroundColor: '#000', alignItems: 'center', justifyContent: 'center' },
  cameraCloseButton: { position: 'absolute', top: 50, right: 20, zIndex: 10, padding: 8 },
  cameraView: { width: 280, height: 380, borderRadius: 18, overflow: 'hidden' },
  captureButton: { backgroundColor: AMBER, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12, marginTop: 24 },
  captureButtonText: { color: '#000', fontWeight: '800', fontSize: 12, textTransform: 'uppercase' },
  ...
  iconBarButton: { padding: 10, borderRadius: 12, backgroundColor: 'rgba(24,24,27,0.6)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  iconBarButtonActive: { backgroundColor: 'rgba(251,191,36,0.1)', borderColor: 'rgba(251,191,36,0.3)' },
  textInput: {
    flex: 1,
