...
export default function AdminDashboard() {
  ...
  const fetchAllDashboardData = async () => {
    try {
      ...
      Toast.show({ type: 'error', text1: 'Failed to load dashboard statistics' });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleDeleteUserProfile = (userId: string, userEmail: string) => {
    ...
    if (targetEmail === 'admin@glowcare.ai' || targetEmail === 'admin@notevault.com') {
      Toast.show({ type: 'error', text1: 'Root admin profile cannot be removed' });
      return;
    }
        ...
        <View style={styles.cardFooter}>
          <View style={[styles.rowGap, styles.flex1]}>
            ...
            <Text style={styles.ownerText} numberOfLines={1}>
              Owner: <Text style={styles.ownerName}>{note.uploadedBy?.name || 'System'}</Text>
            </Text>
          </View>
          <View
            style={[
  ...
  return (
    <View style={styles.screen}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={fetchAllDashboardData} style={styles.refreshButton}>
      ...
      <View style={styles.statsRow}>
        <TouchableOpacity style={styles.statCard} onPress={() => setTab('users')}>
        ...
        <TouchableOpacity style={styles.statCard} onPress={() => setTab('documents')}>
      ...
      <View style={styles.tabRow}>
        <TouchableOpacity
          ...
          style={[styles.tabButton, tab === 'documents' && styles.tabButtonActive]}
        >
...
const styles = StyleSheet.create({
  ...
  tabText: { color: '#a1a1aa', fontWeight: '700', fontSize: 12, textTransform: 'uppercase' },
  ...
  downloadsPill: { backgroundColor: 'rgba(39,39,42,0.5)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  ...
  rolePillAdmin: { backgroundColor: 'rgba(245,158,11,0.1)', borderColor: 'rgba(245,158,11,0.2)' },
  rolePillExpert: { backgroundColor: 'rgba(52,211,153,0.1)', borderColor: 'rgba(52,211,153,0.2)' },
  rolePillStudent: { backgroundColor: '#27272a', borderColor: '#3f3f46' },
  rolePillText: { fontSize: 9, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1 },

  avatarBox: { padding: 10, borderRadius: 12, borderWidth: 1 },
  avatarAdmin: { backgroundColor: 'rgba(245,158,11,0.1)', borderColor: 'rgba(245,158,11,0.2)' },
  avatarExpert: { backgroundColor: 'rgba(52,211,153,0.1)', borderColor: 'rgba(52,211,153,0.2)' },
