...
export default function Upload() {
  ...
  const selectedSubject = subjectsForSemester.find(
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.subjectCode)
      return Toast.show({ type: "error", text1: "Please choose a subject" });
    if (docType === "paper" && !canUploadPapers)
      return Toast.show({ type: "error", text1: "Unauthorized." });
      ...
      <View style={styles.card}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            Share notes or papers with your classmates
          </Text>
        </View>
        ...
        <View style={styles.typeRow}>
          ...
          >
            <NotebookText
              size={16}
              color={docType === "note" ? "#000" : "#a1a1aa"}
            />
            <Text
              style={[
                styles.typeButtonText,
                docType === "note" && { color: "#000" },
              ]}
            >
              Note
            </Text>
          ...
          <TouchableOpacity
            onPress={() => {
              if (!canUploadPapers) {
                Toast.show({
                  type: "error",
                  text1: "Only experts or admins can upload papers.",
                });
                return;
              }
              setDocType("paper");
          ...
          >
            <FileText
              size={16}
              color={docType === "paper" ? "#000" : "#a1a1aa"}
            />
            <Text
              style={[
                styles.typeButtonText,
                docType === "paper" && { color: "#000" },
              ]}
            >
              Paper
            </Text>
        ...
        {!canUploadPapers && (
          <Text style={styles.hintText}>
            Papers can only be uploaded by experts or admins.
          </Text>
        )}
        ...
        <View style={styles.chipRow}>
          {[1, 2, 3, 4].map((s) => (
            <TouchableOpacity
              ...
              style={[
                styles.chip,
                formData.semester === s && styles.chipActive,
              ]}
            >
              <Text
                style={[
        ...
        <View style={styles.chipRowWrap}>
          {subjectsForSemester.map((s) => (
            <TouchableOpacity
              ...
              style={[
                styles.subjectChip,
                formData.subjectCode === s.code && styles.chipActive,
              ]}
            >
              <Text
                style={[
                  styles.chipText,
                  formData.subjectCode === s.code && { color: "#000" },
                ]}
              >
                {s.code}
              </Text>
        ...
        {!!selectedSubject && (
          <Text style={styles.subjectFullName}>{selectedSubject.name}</Text>
        )}
        ...
        <Text style={styles.hintText}>
          You can upload up to 3 files. Individual file limit: 10MB.
        </Text>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.submitButtonText}>
              Upload {docType === "paper" ? "Paper" : "Notes"}
            </Text>
          )}
        </TouchableOpacity>
      </View>
